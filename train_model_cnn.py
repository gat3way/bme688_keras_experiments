...
def load_data(files):
    ...
    for fn in files:
        try:
            ...
            if train_data_out is not None:
                ...
                for i in range(0,t_x):
                    train_data_out[i][current_class] = 1
            ...
            if val_data_out is not None:
                ...
                for i in range(0,v_x):
                    val_data_out[i][current_class] = 1
    ...
    val_data_in = val_data_in.reshape(val_data_in.shape[0], val_data_in.shape[1], 1)
...
def train_network(model_name, train_in, train_out, val_in, val_out):
    ...
    model.add(Conv1D(25, (10), strides=1, dilation_rate=2, activation='relu' , input_shape=(SAMPLES_PER_CYCLE,1), padding='causal',bias_initializer=initializers.Constant(0.1),kernel_initializer='random_normal'))

    #model.add(MaxPooling1D(1,padding='same'))
    #model.add(Conv1D(SAMPLES_PER_CYCLE, (1), activation='relu' , input_shape=(5,1), padding='same',bias_initializer=initializers.Constant(0.1),kernel_initializer='random_normal'))
    
    model.add(Flatten())

    #model.add(BatchNormalization())
    #model.add(Dense(SAMPLES_PER_CYCLE/2, activation='relu',kernel_initializer='random_normal',bias_initializer=initializers.Constant(0.1)))
    ...
    model.fit(train_in, train_out, epochs=500, batch_size=100, shuffle=True)
